...
export const useGameStore = defineStore('game', () => {
  ...
  const items: { id: number, name: string, rarity: Rarity, image: string }[] = [
    ...
    { id: 4, name: 'Desert Eagle', rarity: 'uncommon', image: '/images/deagle.png' },
    { id: 5, name: 'Glock-18', rarity: 'common', image: '/images/glock-18.png' },
  ...
  function initGame(selectedDifficulty: Difficulty, gameSeed?: string) {
    ...
    while (selectedItems.length > 0) {
      const randomIndex = Math.floor(seededRandom(seed.value + selectedItems.length.toString()) * selectedItems.length)
